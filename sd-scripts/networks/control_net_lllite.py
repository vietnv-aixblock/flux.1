...
class ControlNetLLLite(torch.nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"create ControlNet LLLite for U-Net: {len(self.unet_modules)} modules."
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"number of parameters {sum(p.numel() for p in control_net.parameters() if p.requires_grad)}"
    )

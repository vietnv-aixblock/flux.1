...
class Sd3TokenizeStrategy(TokenizeStrategy):
    def __init__(
        self, t5xxl_max_length: int = 256, tokenizer_cache_dir: Optional[str] = None
    ) -> None:
        ...
        self.clip_g = self._load_tokenizer(
            CLIPTokenizer, CLIP_G_TOKENIZER_ID, tokenizer_cache_dir=tokenizer_cache_dir
        )
...
class Sd3TextEncodingStrategy(TextEncodingStrategy):
    ...
    def encode_tokens(
        ...
        if l_tokens is None or clip_l is None:
            ...
            assert (
                g_tokens is not None
            ), "g_tokens must not be None if l_tokens is not None"
            ...
            for i in range(l_tokens.shape[0]):
                drop_l = enable_dropout and (
                    self.l_dropout_rate > 0.0 and random.random() < self.l_dropout_rate
                )
                drop_g = enable_dropout and (
                    self.g_dropout_rate > 0.0 and random.random() < self.g_dropout_rate
                )
                if not drop_l:
            ...
            if len(non_drop_l_indices) == batch_size:
                ...
                l_pooled = torch.zeros(
                    (batch_size, 768), device=clip_l.device, dtype=torch.float32
                )
                l_out = torch.zeros(
                    (batch_size, l_seq_len, 768),
                    device=clip_l.device,
                    dtype=torch.float32,
                )
                l_attn_mask = torch.zeros(
                    (batch_size, l_seq_len),
                    device=clip_l.device,
                    dtype=l_attn_mask.dtype,
                )
                if len(non_drop_l_indices) > 0:
            ...
            if len(non_drop_g_indices) == batch_size:
                ...
                g_pooled = torch.zeros(
                    (batch_size, 1280), device=clip_g.device, dtype=torch.float32
                )
                g_out = torch.zeros(
                    (batch_size, g_seq_len, 1280),
                    device=clip_g.device,
                    dtype=torch.float32,
                )
                g_attn_mask = torch.zeros(
                    (batch_size, g_seq_len),
                    device=clip_g.device,
                    dtype=g_attn_mask.dtype,
                )
                if len(non_drop_g_indices) > 0:
        ...
        if t5xxl is None or t5_tokens is None:
            ...
            for i in range(t5_tokens.shape[0]):
                drop_t5 = enable_dropout and (
                    self.t5_dropout_rate > 0.0
                    and random.random() < self.t5_dropout_rate
                )
                if not drop_t5:
            ...
            if len(non_drop_t5_indices) == batch_size:
                ...
                t5_out = torch.zeros(
                    (batch_size, t5_seq_len, 4096),
                    device=t5xxl.device,
                    dtype=torch.float32,
                )
                t5_attn_mask = torch.zeros(
                    (batch_size, t5_seq_len),
                    device=t5xxl.device,
                    dtype=t5_attn_mask.dtype,
                )
                if len(non_drop_t5_indices) > 0:
...
class Sd3TextEncoderOutputsCachingStrategy(TextEncoderOutputsCachingStrategy):
    ...
    def get_outputs_npz_path(self, image_abs_path: str) -> str:
        return (
            ...
            + Sd3TextEncoderOutputsCachingStrategy.SD3_TEXT_ENCODER_OUTPUTS_NPZ_SUFFIX
        )
    ...
    def is_disk_cached_outputs_expected(self, npz_path: str):
        ...
        try:
            ...
            if "lg_pooled" not in npz:
                return False
            if (
                "clip_l_attn_mask" not in npz or "clip_g_attn_mask" not in npz
    ...
    def cache_batch_outputs(
        ...
        with torch.no_grad():
            ...
            lg_out, t5_out, lg_pooled, l_attn_mask, g_attn_mask, t5_attn_mask = (
                sd3_text_encoding_strategy.encode_tokens(
                    tokenize_strategy,
                    models,
                    tokens_and_masks,
                    apply_lg_attn_mask=self.apply_lg_attn_mask,

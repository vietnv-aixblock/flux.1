...
class SdTokenizeStrategy(TokenizeStrategy):
    def __init__(
        ...
        if v2:
            ...
            self.tokenizer = self._load_tokenizer(
                CLIPTokenizer, TOKENIZER_ID, tokenizer_cache_dir=tokenizer_cache_dir
            )
    ...
    def tokenize(self, text: Union[str, List[str]]) -> List[torch.Tensor]:
        ...
        return [
            torch.stack(
                [self._get_input_ids(self.tokenizer, t, self.max_length) for t in text],
                dim=0,
            )
        ]
    ...
    def tokenize_with_weights(
        ...
        weights_list = []
        for t in text:
            tokens, weights = self._get_input_ids(
                self.tokenizer, t, self.max_length, weighted=True
            )
...
class SdTextEncodingStrategy(TextEncodingStrategy):
    ...
    def encode_tokens(
        ...
        if max_token_length != model_max_length:
            v1 = (
                sd_tokenize_strategy.tokenizer.pad_token_id
                == sd_tokenize_strategy.tokenizer.eos_token_id
            )
            if not v1:
                ...
                for i in range(1, max_token_length, model_max_length):
                    states_list.append(
                        encoder_hidden_states[:, i : i + model_max_length - 2]
                    )  # <BOS> の後から <EOS> の前まで
    ...
    def encode_tokens_with_weights(
        ...
        encoder_hidden_states = self.encode_tokens(
        ...
        if weights.shape[1] == 1:  # no max_token_length
            ...
            encoder_hidden_states = encoder_hidden_states * weights.squeeze(
                1
            ).unsqueeze(2)
        else:
            # weights: ((b, n, 77), (b, n, 77)), hidden_states: (b, n*75+2, 768), (b, n*75+2, 768)
            for i in range(weights.shape[1]):
                encoder_hidden_states[:, i * 75 + 1 : i * 75 + 76] = (
                    encoder_hidden_states[:, i * 75 + 1 : i * 75 + 76]
                    * weights[:, i, 1:-1].unsqueeze(-1)
                )
...
class SdSdxlLatentsCachingStrategy(LatentsCachingStrategy):
    ...
    def __init__(
        ...
        self.sd = sd
        self.suffix = (
            SdSdxlLatentsCachingStrategy.SD_LATENTS_NPZ_SUFFIX
            if sd
            else SdSdxlLatentsCachingStrategy.SDXL_LATENTS_NPZ_SUFFIX
        )
    ...
    def get_latents_npz_path(
        ...
        old_npz_file = (
            ...
            + SdSdxlLatentsCachingStrategy.SD_OLD_LATENTS_NPZ_SUFFIX
        )
    ...
    def is_disk_cached_latents_expected(
        ...
        return self._default_is_disk_cached_latents_expected(
            8, bucket_reso, npz_path, flip_aug, alpha_mask
        )

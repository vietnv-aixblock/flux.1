...
class SdxlTokenizeStrategy(TokenizeStrategy):
    def __init__(
        ...
        self.tokenizer1 = self._load_tokenizer(
            CLIPTokenizer, TOKENIZER1_PATH, tokenizer_cache_dir=tokenizer_cache_dir
        )
        self.tokenizer2 = self._load_tokenizer(
            CLIPTokenizer, TOKENIZER2_PATH, tokenizer_cache_dir=tokenizer_cache_dir
        )
    ...
    def tokenize(self, text: Union[str, List[str]]) -> List[torch.Tensor]:
        ...
        return (
            torch.stack(
                [
                    self._get_input_ids(self.tokenizer1, t, self.max_length)
                    for t in text
                ],
                dim=0,
            ),
            torch.stack(
                [
                    self._get_input_ids(self.tokenizer2, t, self.max_length)
                    for t in text
                ],
                dim=0,
            ),
    ...
    def tokenize_with_weights(self, text: str | List[str]) -> Tuple[List[torch.Tensor]]:
        ...
        for t in text:
            tokens1, weights1 = self._get_input_ids(
                self.tokenizer1, t, self.max_length, weighted=True
            )
            tokens2, weights2 = self._get_input_ids(
                self.tokenizer2, t, self.max_length, weighted=True
            )
...
class SdxlTextEncodingStrategy(TextEncodingStrategy):
    ...
    def encode_tokens(
        ...
        tokenizer1, tokenizer2 = (
            sdxl_tokenize_strategy.tokenizer1,
            sdxl_tokenize_strategy.tokenizer2,
        )
    ...
    def encode_tokens_with_weights(
        ...
        hidden_states1, hidden_states2, pool2 = self.encode_tokens(
        ...
        if weights_list[0].shape[1] == 1:  # no max_token_length
            ...
            for weight, hidden_states in zip(
                weights_list, [hidden_states1, hidden_states2]
            ):
                for i in range(weight.shape[1]):
                    hidden_states[:, i * 75 + 1 : i * 75 + 76] = hidden_states[
                        :, i * 75 + 1 : i * 75 + 76
                    ] * weight[:, i, 1:-1].unsqueeze(-1)
...
class SdxlTextEncoderOutputsCachingStrategy(TextEncoderOutputsCachingStrategy):
    ...
    def is_disk_cached_outputs_expected(self, npz_path: str):
        ...
        try:
            ...
            if (
                "hidden_state1" not in npz
                or "hidden_state2" not in npz
                or "pool2" not in npz
            ):
                return False
    ...
    def cache_batch_outputs(
        ...
        sdxl_text_encoding_strategy = (
            text_encoding_strategy
        )  # type: SdxlTextEncodingStrategy
        ...
        if self.is_weighted:
            ...
            with torch.no_grad():
                hidden_state1, hidden_state2, pool2 = (
                    sdxl_text_encoding_strategy.encode_tokens_with_weights(
            ...
            with torch.no_grad():
                hidden_state1, hidden_state2, pool2 = (
                    sdxl_text_encoding_strategy.encode_tokens(

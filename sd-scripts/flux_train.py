...
def extract_training_args(input_args=None):
    parser = argparse.ArgumentParser()
    parser.add_argument("--channel_log", type=str, default=None, required=False)
    args, _ = parser.parse_known_args()

    log_queue, _ = start_queue(args.channel_log)
    write_log(log_queue)
    remove_arg("--channel_log")


def remove_arg(flag):
    if flag in sys.argv:
        index = sys.argv.index(flag)
        if index + 1 < len(sys.argv):  # Ensure there's a value after the flag
            sys.argv.pop(index + 1)  # Remove the value
        sys.argv.pop(index)

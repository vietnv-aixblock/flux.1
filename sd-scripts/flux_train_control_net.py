...
def train(args):
    ...
    if args.dataset_class is None:
        blueprint_generator = BlueprintGenerator(
            ConfigSanitizer(False, False, True, True)
        )
    ...
    if is_swapping_blocks:
        ...
        flux.move_to_device_except_swap_blocks(
            accelerator.device
    ...
    if args.deepspeed:
        ...
        controlnet = accelerator.prepare(
            controlnet
        )  # , device_placement=[not is_swapping_blocks])
    ...
    if is_main_process:
        flux_train_utils.save_flux_model_on_train_end(
            args, save_dtype, epoch, global_step, controlnet
        )
